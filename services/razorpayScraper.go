...
func RazorpayScraper() ([]models.Job, error) {
	client := &http.Client{}
	req, err := http.NewRequest("GET", "https://boards-api.greenhouse.io/v1/boards/razorpaysoftwareprivatelimited/jobs?content=true", nil)
	if err != nil {
		return nil, err
	}
	...
	var apiResponse models.RazorpayData
	...
	for _, item := range apiResponse.Jobs {
		desc := item.Content + "\n\n" +
			"Employment Type: " + item.Employment + "\n" +
